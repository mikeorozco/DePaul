...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def funcL(a: String, b: String): String = a + b + term
    xs match {
    	  case Nil => ""
	  case y::ys => foldLeft(ys, funcL(y,""), funcL)
    }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    // TODO: Provide definition here.
    def funcR(a: String, b: String) : String = delimiter + a + b
    xs match {
    	  case Nil => ""
	  case z::zs => z + foldRight(zs, delimiter, funcR)
    }
  }
  ...
  def stringLengths (xs:List[String]) : List[(String, Int)] = {
    // TODO: Provide definition here.
    def len (word: String) : (String, Int) = (word, word.length)
    if (xs == Nil) Nil
    else map(xs, len)
  }
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = {
    ...
    ys match {
    	  case Nil => Nil
	  case g::gs => if ( x != g) g :: (delete1(x,gs))
	       	       else  (delete1(x, gs))  
    }
  }
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
    ...
    xs match{
       case Nil => Nil
       case a::b::as if (a == b) => removeDupes1(b::as)
       case a::as => a::removeDupes1(as)
    }  
  }
  ...
  def removeDupes2 [X] (xs:List[X]) : List[(Int, X)] = {
    ...
    xs match{
       ...
       case Nil => Nil
       //case a::b::as if (a == b) => (count + 1, a)::removeDupes2(b::as)
       //case a::b::as if (a != b)=> removeDupes2(b::as)
       case y::ys => 
         val partial:List[(Int, X)] = removeDupes2(ys)
         partial match {
           case (n,v):: rest if (y == v) => (n+1, v):: rest
           case _ => (1, y) :: partial
         }
    }
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
    ...
    (n, xs) match{
       case (_, Nil) => (Nil, Nil)
       case (n, y::ys) => { val (list1, list2) = splitAt(n-1, ys)
       	    if (n>0) (y::list1, list2)
	    else (list1, y::list2)
	    }
    }
  }
