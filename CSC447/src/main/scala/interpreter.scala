...
object interpreter {
  ...
  def evaluate (env:Env, e:Exp) : Val = {
    ...
    val result:Val = e match {
      ...
      case ExpIf (e, a, b) =>
        val x: Val = evaluate(env, e) 
          x match {
            case ValInt(0) => evaluate ( env, b)
            case _ => evaluate (env, a)
          }
  ...
  def prettyPrintExp (e:Exp) : String = e match {
    ...
    // TODO: Add case for ExpIf below.
    case ExpIf (e,a,b) => "(expression: " + prettyPrintExp(e) + "true " + prettyPrintExp(a) + "/false: " + prettyPrintExp(b) + ")"
  }
  ...
  // TODO: Change the body of the definition below.
  val exX:Env = //todoEnv // TODO 
  {
    val env: Env = scala.collection.immutable.Map.empty
    env + ( ( "x", ValInt(50)))
  }
